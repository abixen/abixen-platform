...
package com.abixen.platform.service.businessintelligence.multivisualisation.application.service.parser.impl.validator;
...
public class FileDataValidator {
    ...
    @Autowired
    public FileDataValidator(DataTypeValidator dataTypeValidator) {
    ...
    private void validateAs(final FileParserMessage<DataFileColumn> msg, final List<DataValueType> columnTypes, final int i, final int j, final String element) {
        switch (columnTypes.get(j)) {
            ...
            case STRING:
                ...
                break;
            default: throw new PlatformRuntimeException(format("[Row: %s Cell: %s ]Column type not recognized", i, j));
        }
    }
}
